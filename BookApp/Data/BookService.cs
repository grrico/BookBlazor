...
namespace BookApp.Data
{
    public class BookService : IBookService
    {
        ...
        public Task<long> Create(Book book)
        {
            ...
            var bookId = Task.FromResult(_dapperService.Insert<long>("[dbo].[spAddBook]", dbPara,commandType: CommandType.StoredProcedure));
            return bookId;
        }
        public Task<Book> ReadByPk(long isbn)
        {
            var book = Task.FromResult(_dapperService.Get<Book>
                ($"select * from [Book] where ISBN = {isbn}",null, commandType: CommandType.Text));
            return book;
        }
        public Task<int> Update(Book book, long pk)
        {
            ...
            var updateBook = Task.FromResult(_dapperService.Update<int>("[dbo].[spUpdateBook]", dbPara,commandType: CommandType.StoredProcedure));
            return updateBook;
        }
        public Task<int> Delete(long id)
        {
            var deleteBook = Task.FromResult(_dapperService.Execute
                ($"Delete [Book] where ISBN = {id}", null,commandType: CommandType.Text));
            return deleteBook;
        }
        public Task<int> Count(string search)
        {
            var totBook = Task.FromResult(_dapperService.Get<int>
               ($"select COUNT(ISBN) from [Book] WHERE Title like '%{search}%'", null,
        ...
        /// <returns></returns>
        public Task<List<BookAuPub>> ListAll(int skip,int take, string orderBy, string direction = "DESC",string search = "")
        {
